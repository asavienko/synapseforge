...
export function ActivityLogTab({
  ...
  return (
    <div className="glow-border rounded-2xl bg-white dark:bg-white/[0.02] overflow-hidden">
      <div className="p-5 border-b border-gray-200 dark:border-white/5 flex flex-col sm:flex-row sm:items-center gap-3 justify-between">
        <div className="flex items-center gap-2">
          <h3 className="text-sm font-semibold text-gray-900 dark:text-white">{t("activity.title")}</h3>
          {logsLoading && <Loader2 className="w-3.5 h-3.5 text-gray-400 dark:text-zinc-600 animate-spin" />}
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          <select
            value={logFilter}
            onChange={(e) => setLogFilter(e.target.value as typeof logFilter)}
            className="text-xs bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg px-2.5 py-1.5 text-gray-700 dark:text-zinc-300 focus:outline-none focus:border-blue-500 transition-colors"
          >
          ...
          <span className="text-xs text-gray-400 dark:text-zinc-600">{t("activity.autoRefresh")}</span>
          <button onClick={loadLogs} className="text-xs text-gray-500 dark:text-zinc-500 hover:text-white transition-colors">{t("activity.refresh")}</button>
          {filteredLogs.length > 0 && (
            <button
              ...
              className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-zinc-400 hover:text-blue-400 transition-colors"
              title="Export logs"
            >
      ...
      {logsLoading && logs.length === 0 ? (
        ...
        <div className="p-8 text-center">
          ...
          <p className="text-gray-500 dark:text-zinc-500 text-sm">{t("activity.noActivity")}</p>
        </div>
      ) : filteredLogs.length === 0 ? (
        <div className="p-8 text-center">
          <Activity className="w-8 h-8 text-zinc-700 mx-auto mb-2" />
          <p className="text-gray-500 dark:text-zinc-500 text-sm">{t("activity.noResults")}</p>
          <button onClick={() => setLogFilter("all")} className="mt-2 text-xs text-blue-600 dark:text-blue-400 hover:text-blue-300">{t("activity.clearFilter")}</button>
        </div>
      ) : (
        <div>
          {groupLogsByDay(filteredLogs).map(({ label, logs: dayLogs }) => (
            <div key={label}>
              <div className="px-4 py-2 bg-white/[0.01] border-b border-gray-200 dark:border-white/5">
                <span className="text-xs font-semibold text-gray-500 dark:text-zinc-500 uppercase tracking-wider">{label}</span>
              </div>
              <div className="divide-y divide-gray-200 dark:divide-white/5">
                {dayLogs.map((log) => {
                  ...
                  meta = meta ?? { icon: "·", color: "text-gray-500 dark:text-zinc-400" };
                  ...
                  return (
                    <div key={log.id} className="flex items-start gap-4 p-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {log.details && <div className="text-xs text-gray-500 dark:text-zinc-500 mt-0.5">{log.details}</div>}
                      </div>
                      <div title={absTime} className="text-xs text-gray-400 dark:text-zinc-600 shrink-0 cursor-default">{relTime}</div>
                    </div>
                  );
                })}
              </div>
