...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (data.managerId && user.manager && user.email) {
    ...
    emailService.newUserAlert(user.manager.email, user.manager.name, user.name ?? user.email, user.email, user.onboardingData as { businessName?: string; industry?: string; useCase?: string; teamSize?: string; agentType?: string } | null ?? undefined).catch(console.error);
  }
