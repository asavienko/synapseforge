  ...
  return (
    <Link
      ...
      className={cn(
        ...
        isActive
          ? "bg-blue-600/20 text-gray-900 dark:text-white border border-blue-500/20"
...
function ExternalNavItem({ href, icon: Icon, label }: { href: string; icon: React.ElementType; label: string }) {
  return (
    <a
      ...
      className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-colors text-gray-500 dark:text-zinc-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-white/5"
    >
      <Icon className="w-4 h-4" />
      <span className="flex-1">{label}</span>
    </a>
    ...
    <div className="flex flex-col h-full">
      <div className="p-4 border-b border-gray-200 dark:border-white/5 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2" onClick={onClose}>
          <HelixLogo className="w-5 h-5 text-blue-400" size={20} />
          <span className="font-bold text-sm tracking-tight text-gray-900 dark:text-white">OpenHelix AI</span>
        </Link>
        <div className="flex items-center gap-1">
          {!onClose && <NotificationCenter />}
          {onClose && (
            <button onClick={onClose} className="text-gray-500 dark:text-zinc-500 hover:text-gray-900 dark:hover:text-white transition-colors md:hidden">
      ...
      <nav className="flex-1 p-4 space-y-1">
        ...
        {isAdmin && (
          <div className={isManager ? "mt-1" : "pt-2 mt-2 border-t border-gray-200 dark:border-white/5"}>
      ...
      <div className="p-4 border-t border-gray-200 dark:border-white/5">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-gray-500 dark:text-zinc-500 truncate">{userEmail}</div>
          </div>
        </div>
        <button
          onClick={() => signOut({ callbackUrl: `${typeof window !== "undefined" ? window.location.origin : ""}/en/sign-in`, redirect: true })}
          className="flex items-center gap-2 text-xs text-gray-500 dark:text-zinc-500 hover:text-gray-900 dark:hover:text-white transition-colors py-2 px-3 rounded-lg hover:bg-gray-100 dark:hover:bg-white/5 w-full"
        >
        ...
        <button
          ...
          className="mt-2 flex items-center justify-between text-xs text-gray-500 dark:text-zinc-600 hover:text-gray-700 dark:hover:text-zinc-400 transition-colors py-2 px-3 rounded-lg hover:bg-gray-100 dark:hover:bg-white/5 w-full border border-gray-200 dark:border-white/5 border-dashed"
        >
          <span>{t("commandPalette")}</span>
          <kbd className="flex items-center gap-0.5 px-1.5 py-0.5 bg-gray-100 dark:bg-white/5 rounded text-[10px]">
...
export function DashboardSidebar({ userName, userEmail, isAdmin, isManager }: SidebarProps) {
  ...
  return (
    <>
      {/* Mobile top bar */}
      <div className="md:hidden flex items-center justify-between px-4 h-14 border-b border-gray-200 dark:border-white/5 bg-white dark:bg-[#0a0a0f] fixed top-0 left-0 right-0 z-40 safe-area-inset">
        ...
        <div className="flex items-center gap-2">
          ...
          <NotificationCenter />
          <button onClick={() => setMobileOpen(true)} className="text-gray-500 dark:text-zinc-400 hover:text-gray-900 dark:hover:text-white transition-colors p-2 -mr-2">
      ...
      {/* Mobile drawer */}
      <div className={cn(
        "md:hidden fixed top-0 left-0 bottom-0 w-64 bg-white dark:bg-[#0a0a0f] border-r border-gray-200 dark:border-white/5 z-50 transition-transform duration-200 safe-area-inset",
      ...
      {/* Desktop sidebar */}
      <aside className="hidden md:flex w-64 border-r border-gray-200 dark:border-white/5 flex-col shrink-0">
