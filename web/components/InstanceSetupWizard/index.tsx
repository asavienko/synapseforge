...
export function InstanceSetupWizard({ onClose, onCreated }: WizardProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 px-4 py-6">
      <div className="bg-[#111118] border border-gray-200 dark:border-white/10 rounded-2xl w-full max-w-2xl shadow-2xl flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200 dark:border-white/5 shrink-0">
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      i < stepIndex
                        ...
                        : i === stepIndex
                        ? "bg-blue-600/30 border border-blue-500 text-blue-600 dark:text-blue-300"
                        : "bg-gray-50 dark:bg-white/5 text-gray-400 dark:text-zinc-600"
                    )}
                  >
                    {i < stepIndex ? <Check className="w-3 h-3" /> : i + 1}
                  </button>
                  {i < STEPS.length - 1 && (
                    <div className={cn("w-3 sm:w-6 h-0.5 rounded shrink-0", i < stepIndex ? "bg-blue-600" : "bg-gray-200 dark:bg-white/10")} />
                  )}
                </div>
              ))}
        ...
        {/* Footer */}
        {!success && (
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-6 border-t border-gray-200 dark:border-white/5 shrink-0">
            <button
              onClick={stepIndex === 0 ? onClose : goPrev}
              className="flex items-center justify-center gap-2 text-sm text-zinc-400 hover:text-white transition-colors px-4 py-2.5 rounded-xl border border-gray-200 dark:border-white/10 hover:border-white/20"
            >
            ...
            {step !== "deploy" ? (
              <button
                onClick={goNext}
                disabled={!canGoNext}
                className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-600 disabled:opacity-40 transition-colors px-4 py-2.5 rounded-xl text-sm font-semibold text-white"
              >
                Next
              ...
              <button
                ...
                disabled={submitting}
                className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-600 disabled:opacity-40 transition-colors px-4 sm:px-5 py-2.5 rounded-xl text-sm font-semibold text-white whitespace-nowrap"
              >
