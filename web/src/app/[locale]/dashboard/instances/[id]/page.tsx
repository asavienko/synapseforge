...
export default function InstanceDetailPage() {
  ...
  // WhatsApp (Twilio) wizard state
  type WhatsAppWizardStep = "form" | "verifying" | "verified" | "live" | "error";
  const [waStep, setWaStep] = useState<WhatsAppWizardStep>("form");
  const [waForm, setWaForm] = useState({ accountSid: "", authToken: "", number: "" });
  const [waError, setWaError] = useState<string | null>(null);
  const [waLiveNumber, setWaLiveNumber] = useState<string | null>(null);
  ...
  async function waVerify() {
    if (!waForm.accountSid || !waForm.authToken || !waForm.number) return;
    setWaStep("verifying");
    setWaError(null);
    const res = await fetch(`/api/instances/${id}/setup-whatsapp`, {
      ...
      body: JSON.stringify({
        ...
        authToken: waForm.authToken,
        whatsappNumber: waForm.number,
      }),
    });
    const data = await res.json() as { ok?: boolean; error?: string; number?: string };
    if (!res.ok) {
      setWaError(data.error ?? t("credentials.saveFailed"));
      setWaStep("error");
    } else {
      setWaLiveNumber(data.number ?? waForm.number);
      setWaStep("verified");
      loadCredentials();
    }
  }
  ...
  async function waSendTest() {
    ...
    setWaTestSent(false);
    const res = await fetch(`/api/instances/${id}/test-whatsapp`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
print(resp.choices[0].message.content)`}</pre>
      ...
      {tab === "Credentials" && (
        <div className="space-y-5">
          ...
          {/* ── WhatsApp via Twilio — 3-step wizard ── */}
          {(() => {
            const hasWhatsapp = credentials.some((c) => c.key === "twilio_account_sid");
            const storedNumber = credentials.find((c) => c.key === "twilio_whatsapp_number")?.maskedValue ?? null;
            // Determine effective step: if creds already exist and wizard is at "form", show "live"
            const effectiveStep: typeof waStep = hasWhatsapp && waStep === "form" ? "live" : waStep;

            // Step indicator component (inline)
            const steps = [
              t("credentials.whatsapp.wizardStep1"),
              t("credentials.whatsapp.wizardStep2"),
              t("credentials.whatsapp.wizardStep3"),
            ];
            const activeStepIndex = effectiveStep === "form" ? 0
              : effectiveStep === "verifying" || effectiveStep === "verified" || effectiveStep === "error" ? 1
              : 2; // "live"
            ...
            return (
              <div className="glow-border rounded-2xl bg-white/[0.02] overflow-hidden">
                ...
                <div className="p-4 border-b border-white/5 flex items-center gap-3">
                  ...
                  {hasWhatsapp && effectiveStep !== "form" && (
                    <span className="text-xs px-2 py-1 rounded-full bg-emerald-500/15 text-emerald-400 border border-emerald-500/20">
                ...
                <div className="p-4 space-y-4">
                    ...
                    {steps.map((label, i) => (
                      <div key={i} className="flex items-center gap-1 sm:gap-2 min-w-0">
                        <div className={cn(
                          ...
                          {i < activeStepIndex ? <Check className="w-3 h-3" /> : i + 1}
                        </div>
                        <span className={cn(
                          "text-xs truncate hidden sm:block",
                          i === activeStepIndex ? "text-violet-300 font-medium" : "text-zinc-500"
                        )}>
                          {label}
                        </span>
                        {i < steps.length - 1 && (
                          <div className={cn(
                            ...
                            i < activeStepIndex ? "bg-violet-600" : "bg-white/10"
                          )} />
                        )}
                      </div>
                    ))}
                  ...
                  {effectiveStep === "form" && (
                    <div className="bg-zinc-900 border border-white/5 rounded-2xl p-4 sm:p-6 space-y-3">
                      ...
                      <div className="space-y-1">
                        <label className="text-xs text-zinc-400">{t("credentials.whatsapp.accountSidLabel")}</label>
                        <input
                          type="text"
                      ...
                      <div className="space-y-1">
                        <label className="text-xs text-zinc-400">{t("credentials.whatsapp.authTokenLabel")}</label>
                        <input
                          type="password"
                      ...
                      <div className="space-y-1">
                        <label className="text-xs text-zinc-400">{t("credentials.whatsapp.whatsappNumberLabel")}</label>
                        <input
                          type="text"
                          value={waForm.number}
                          onChange={(e) => setWaForm(f => ({ ...f, number: e.target.value }))}
                      ...
                      <p className="text-xs text-zinc-500">
                        ...
                        >
                          {t("credentials.whatsapp.getCredentialsHint")}
                        </a>
                      </p>
                      <button
                        onClick={waVerify}
                        disabled={!waForm.accountSid || !waForm.authToken || !waForm.number}
                  ...
                  {effectiveStep === "verifying" && (
                    <div className="bg-zinc-900 border border-white/5 rounded-2xl p-4 sm:p-6 flex flex-col items-center gap-3 py-8">
                      <Loader2 className="w-8 h-8 text-violet-400 animate-spin" />
                      <p className="text-sm text-zinc-300">{t("credentials.whatsapp.verifying")}</p>
                    </div>
                  )}

                  {/* ── Step 2: Verified (send test) ── */}
                  {effectiveStep === "verified" && (
                    <div className="bg-zinc-900 border border-white/5 rounded-2xl p-4 sm:p-6 space-y-4">
                      <div className="flex items-center gap-2 text-emerald-400">
                        <Check className="w-5 h-5" />
                        <span className="text-sm font-medium">{t("credentials.whatsapp.credentialsSaved")}</span>
                      </div>
                      <div className="space-y-2">
                        <p className="text-xs text-zinc-400">{t("credentials.whatsapp.sendTestMessage")}</p>
                        <div className="flex flex-col sm:flex-row gap-2">
                          <input
                            type="text"
                            value={waTestPhone}
                            onChange={(e) => setWaTestPhone(e.target.value)}
                            placeholder={t("credentials.whatsapp.testPhonePlaceholder")}
                          ...
                          >
                            {waTestSending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}
                            {t("credentials.whatsapp.sendTest")}
                          </button>
                        </div>
                        {waTestSent && (
                          <p className="text-xs text-emerald-400 flex items-center gap-1">
                            <Check className="w-3 h-3" /> {t("credentials.whatsapp.testSent")}
                          </p>
                        )}
                      </div>
                      <button
                        onClick={() => setWaStep("live")}
                  ...
                  {effectiveStep === "error" && (
                    <div className="bg-zinc-900 border border-white/5 rounded-2xl p-4 sm:p-6 space-y-3">
                      <div className="flex items-center gap-2 text-red-400">
                        ...
                        <span className="text-sm font-medium">{waError}</span>
                      </div>
                      <button
                        onClick={() => { setWaStep("form"); setWaError(null); }}
                        className="text-sm text-violet-400 hover:text-violet-300 transition-colors"
                      >
                        {t("credentials.whatsapp.backToFix")}
                      </button>
                    </div>
                  )}
                  ...
                  {effectiveStep === "live" && (
                    <div className="bg-zinc-900 border border-white/5 rounded-2xl p-4 sm:p-6 space-y-4">
                      <div className="text-center space-y-1">
                        ...
                        <h4 className="text-base font-semibold text-white">{t("credentials.whatsapp.liveTitle")}</h4>
                        <p className="text-xs text-zinc-400">{t("credentials.whatsapp.liveHint")}</p>
                      </div>
                      {(waLiveNumber ?? storedNumber) && (
                        <div className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                          <span className="text-sm font-mono text-zinc-200 break-all">{waLiveNumber ?? storedNumber}</span>
                          <button
                            onClick={() => {
                              const num = waLiveNumber ?? storedNumber ?? "";
                          ...
                          >
                            ...
                            {t("credentials.whatsapp.copyNumber")}
                          </button>
                        </div>
                      )}
                      <div className="flex flex-col sm:flex-row gap-2">
                        <button
                          onClick={() => {
                            setWaStep("form");
                            setWaForm({ accountSid: "", authToken: "", number: "" });
                            setWaError(null);
                            setWaTestSent(false);
                          }}
                        ...
                        >
                          {t("credentials.whatsapp.reconfigure")}
                        </button>
                        <button
                          onClick={async () => {
                            ...
                            setWaStep("form");
                            setWaLiveNumber(null);
                            loadCredentials();
                          }}
                          className="flex-1 text-xs text-red-400 hover:text-red-300 bg-red-500/10 px-4 py-2.5 rounded-xl transition-colors"
                        >
                          {t("credentials.disconnect")}
                        </button>
                      </div>
